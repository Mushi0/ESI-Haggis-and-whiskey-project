...
chosen_assign = pd.read_csv('Solution\salida_X.csv')
chosen = list(chosen_assign['facility'].unique())
assign = []
i = 0
temp = []
for index, row in chosen_assign.iterrows():
    if index == len(chosen_assign) - 1:
        assign.append(temp)
    if chosen.index(row['facility']) == i:
        temp.append(row['client'] - 1)
    else:
        assign.append(temp)
        temp = []
        i += 1
    # assign[chosen.index(row['facility'])].append(row['client'] - 1)
...
for i, fac in enumerate(chosen):
    data.loc[fac, 'capacity_used'] = (sum([data['total_demand'][j] for j in assign[i]]))/data_capacity[fac]*100
